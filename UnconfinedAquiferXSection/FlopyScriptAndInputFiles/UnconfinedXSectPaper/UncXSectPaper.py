...
nlay, nrow, ncol = 19, 1, 33  # specify the number of layers, rows, and columns

A_i = np.loadtxt("A_i.txt",delimiter='\t').reshape(nlay,nrow, ncol)
...
delr = 10.0
delc = 10.0  # grid spacing along rows and columns
delz = 5.0
...
strt = 72 # used to set the prescribed at the boundaries and the initial head to start the iterations
hnoflo = -99.00
...
#The vertical and horizontal conductivities have to be set to zero to ensure that the tilted impermeable bottom is kept
#   otherwise, the inactive cells indicated in the ibound matrix are rewetted, because the bottom layers are rewetable
vka[15,0,0:8] = vka[16,0,8:16] = vka[17,0,16:24] = 0.0
hk[16,0,7] = hk[17,0,15] = hk[18,0,23] = 0.0
...
lays, rows, cols = np.where(A_W != 0)
#change sign to make extraction negative recharges as required by the wel package
wel_data = np.column_stack((lays, rows, cols, -A_W[lays, rows, cols]))
...
lays, rows, cols = np.where(A_R != 0)
riv_data = np.column_stack((lays, rows, cols, A_hR[lays, rows, cols],A_R[lays, rows, cols],A_hB[lays, rows, cols]))
...
rech = 0.001 #for a vertical cross-section is complicated to translate the input D_QN data into the values neede by ModflowRch
...
pcg = flopy.modflow.ModflowPcg(mf,hclose=1e-7)
