...
A_Ky = np.genfromtxt("A_Ky.txt",delimiter='\t',filling_values=0).reshape(nlay,nrow, ncol)
#Well extraction
...
bottom = A_Bot  #array with nlay x nrow x ncol elements with the bottom elevation
ztop = A_Bot[0,:,:]+40 #arbitrarily set the top elevation 40 m above the bottom

fig, ax = plt.subplots()
ax.imshow(bottom[0,:,:])  
plt.show()  #show modeltop with the bump in the lower center
...
lpf = flopy.modflow.ModflowLpf(mf,hk=hk_rows,vka=vka,chani=-1,hani=hani,ipakcb=53,laytyp=1,laywet=1,wetdry=-0.01,
                               iwetit=1,wetfct=0.1,ihdwet=0)
